...
Style inlineCSSToStyle(String? inlineStyle) {
  ...
  Map<String, List<css.Expression>> declarations = DeclarationVisitor().getDeclarations(sheet);
...
class ExpressionMapping {
  ...
  static FontSize? expressionToFontSize(css.Expression value) {
    if (value is css.NumberTerm) {
      ...
      return FontSize.percent(int.tryParse(value.text)??0);
    } else if (value is css.EmTerm) {
      return FontSize.em(double.tryParse(value.text)??0);
    } else if (value is css.RemTerm) {
      return FontSize.rem(double.tryParse(value.text)??0);
    } else if (value is css.LengthTerm) {
      return FontSize(double.tryParse(value.text.replaceAll(new RegExp(r'\s+(\d+\.\d+)\s+'), ''))??0);
  ...
  static LineHeight expressionToLineHeight(css.Expression value) {
    if (value is css.NumberTerm) {
      return LineHeight.number(double.tryParse(value.text)??0);
    } else if (value is css.PercentageTerm) {
      return LineHeight.percent(double.tryParse(value.text)??0);
    } else if (value is css.EmTerm) {
      return LineHeight.em(double.tryParse(value.text)??0);
    } else if (value is css.RemTerm) {
      return LineHeight.rem(double.tryParse(value.text)??0);
    } else if (value is css.LengthTerm) {
  ...
  static TextDecoration expressionToTextDecorationLine(List<css.Expression> value) {
    List<TextDecoration> decorationList = [];
    for (css.Expression term in value) {
      if(term is css.LiteralTerm) {
        switch(term.text) {
          case "overline":
  ...
  static TextDecorationStyle expressionToTextDecorationStyle(css.Expression value) {
    if (value is css.LiteralTerm) {
      switch(value.text) {
        case "wavy":
  ...
  static List<Shadow> expressionToTextShadow(List<css.Expression> value) {
    ...
    for (List<css.Expression> list in valueList) {
      ...
      css.Expression? exp3 = list.length > 2 ? list[2] : null;
      css.Expression? exp4 = list.length > 3 ? list[3] : null;
      ...
      if (exp is css.LiteralTerm && exp2 is css.LiteralTerm) {
        if (exp3 != null && (exp3 is css.HexColorTerm || exp3 is css.FunctionTerm)) {
          shadow.add(Shadow(
              color: expressionToColor(exp3)??Color(0xFF000000),
              offset: Offset((double.tryParse(exp.text.replaceAll(nonNumberRegex, '')))??0, (double.tryParse(exp2.text.replaceAll(nonNumberRegex, '')))??0)
          ));
        } else if (exp3 != null && exp3 is css.LiteralTerm) {
          if (exp4 != null && (exp4 is css.HexColorTerm || exp4 is css.FunctionTerm)) {
            shadow.add(Shadow(
                color: expressionToColor(exp4)??Color(0xFF000000),
                offset: Offset((double.tryParse(exp.text.replaceAll(nonNumberRegex, '')))??0, (double.tryParse(exp2.text.replaceAll(nonNumberRegex, '')))??0),
                blurRadius: double.tryParse(exp3.text.replaceAll(nonNumberRegex, ''))??0
            ));
          } else {
            shadow.add(Shadow(
                offset: Offset((double.tryParse(exp.text.replaceAll(nonNumberRegex, '')))??0, (double.tryParse(exp2.text.replaceAll(nonNumberRegex, '')))??0),
